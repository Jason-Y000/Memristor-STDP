...
V_probe = 0.75; % Voltage of the probe pulse
...
for n = 1:size(t,2)
    ...
    elseif ((t(n)>=t_before(6)) && (t(n)<t_before(length(t_before)-5)))||((t(n)>=t_after(2)) && (t(n)<t_after(length(t_after)-9)))
        V = cat(2,V,V_probe);
    else
        V = cat(2,V,V_start);
    end
...
for i = 2:size(p,1)
    ...
    if ((p(i,1)>=t_before(6)) && (p(i,1)<t_before(length(t_before)-5)))||((p(i,1)>=t_after(2)) && (p(i,1)<t_after(length(t_after)-10)))
        end_V = abs(p_prime(i,2)-p(i,2));
    end
    if ((p(i-1,1)>=t_before(6)) && (p(i-1,1)<t_before(length(t_before)-5)))||((p(i-1,1)>=t_after(2)) && (p(i-1,1)<t_after(length(t_after)-10)))
